...
function calcPrice(km) {
  const base = 40;
  const firstKm = Math.min(km, 8);
  const extraKm = Math.max(km - 8, 0);
  const firstCost = firstKm * 20;
  const extraCost = extraKm * 10;
  return {
    base,
    firstKm,
    extraKm,
    firstCost,
    extraCost,
    total: Math.round(base + firstCost + extraCost),
  };
}
...
const URGENCIES = [
  { id: "urgent", label: "דחוף", hint: "תוך 4 שעות", Icon: Zap, color: "var(--lime)" },
  { id: "regular", label: "רגיל", hint: "1–2 ימי עסקים", Icon: Clock, color: "var(--teal)" },
];
...
export default function App() {
  ...
  // Load order history once
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await window.storage.get("orders", false);
        if (!cancelled && res && res.value) setOrders(JSON.parse(res.value));
      } catch (e) {
        // no saved orders yet — fine
      } finally {
  ...
  const price = calcPrice(effectiveKm);
  ...
  const handleValidate = useCallback(() => {
    ...
    try { setOrders(next); window.storage.set("orders", JSON.stringify(next), false); } catch(e) {}

    return url;
  }, [fromQuery, toQuery, phoneFrom, phoneTo, notes, size, urgency, effectiveKm, haveAuto, price.total, orders]);
  ...
  const handleDelete = useCallback(
    async (id) => {
      ...
      setOrders(next);
      try {
        await window.storage.set("orders", JSON.stringify(next), false);
      } catch (e) {
        // best effort
      }
    },
    ...
    <div className="app-root" dir="rtl">
      ...
      <div className="wrap">
          ...
          <div className="grid-2">
            {URGENCIES.map(({ id, label, hint, Icon, color }) => {
              ...
              return (
                <div
                  key={id}
                  className={`tile ${sel ? (id === "urgent" ? "sel-urgent" : "sel-regular") : ""}`}
        ...
        <div className="card">
          ...
          <div className="price-row">
            <span className="lbl">{price.firstKm.toFixed(1)} ק״מ ראשונים × ₪20</span>
            <span className="val f-mono">₪{price.firstCost.toFixed(0)}</span>
          </div>
          {price.extraKm > 0 && (
            <div className="price-row">
              <span className="lbl">{price.extraKm.toFixed(1)} ק״מ נוספים (מעבר ל‑8) × ₪10</span>
              <span className="val f-mono">₪{price.extraCost.toFixed(0)}</span>
            </div>
          )}
        ...
        {/* History */}
        <div className="card">
          <div className="section-title"><span>ההזמנות שלי</span></div>
          {!loaded ? (
            <div className="empty-state">טוען...</div>
          ) : orders.length === 0 ? (
            <div className="empty-state">אין הזמנות עדיין. ההזמנה הראשונה שלך תופיע כאן.</div>
          ) : (
            <div>
              {orders.map((o) => {
                const s = SIZES.find((x) => x.id === o.size);
                const u = URGENCIES.find((x) => x.id === o.urgency);
                  ...
                  <div className="order-row" key={o.id}>
                    ...
                    <div className="order-main">
                      ...
                      <div className="order-meta">
                        <span className={`badge ${u && u.id === "urgent" ? "badge-urgent" : "badge-regular"}`}>
                          {u ? u.label : "—"}
                        </span>
                        <span className="f-mono">{o.km} ק״מ</span>
                        <span>{timeAgo(o.createdAt)}</span>
                      </div>
                    </div>
                    <div style={{ textAlign: "left" }}>
                      <div className="order-price f-mono">₪{o.price}</div>
                      <button className="order-del" onClick={() => handleDelete(o.id)} aria-label="מחק הזמנה">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
